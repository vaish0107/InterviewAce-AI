...
export function InterviewSession() {
  ...
  useEffect(() => { setAnswer(question?.answerText || ''); setSaved(false) }, [question?.id, question?.answerText])
  useEffect(() => {
    let active = true
    setEvaluation(null)
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    <Card className="p-5 sm:p-7"><div className="flex flex-wrap gap-2"><Badge>{question.category}</Badge>{question.skill && <Badge>{question.skill}</Badge>}<Badge>{question.difficulty}</Badge></div><h3 className="mt-6 text-xl font-bold leading-8">{question.questionText}</h3><label htmlFor="interview-answer" className="mt-7 block text-sm font-medium">Your answer</label><textarea id="interview-answer" value={answer} onChange={event => { setAnswer(event.target.value); setSaved(false) }} rows={9} placeholder="Write your answer here..." className="mt-2 w-full resize-y rounded-xl border border-slate-300 bg-white p-3.5 text-sm leading-6 outline-none focus:border-indigo-500 focus:ring-3 focus:ring-indigo-500/15 dark:border-slate-700 dark:bg-slate-900" /><div className="mt-4 flex flex-wrap items-center gap-3"><Button onClick={() => void save()} isLoading={saving} disabled={!answer.trim()}><FiCheck />{saving ? 'Saving...' : 'Save Answer'}</Button>{saved && <span role="status" className="text-sm font-medium text-emerald-600">Answer saved</span>}</div></Card>
...
function CompletedSession({ session, index, setIndex, evaluation, evaluationLoading, evaluating, onEvaluate, error }: CompletedProps) {
  const question = session.questions[index]; const unanswered = session.totalQuestions - session.answeredQuestions
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    {error && <Alert message={error} />}
    <Card className="p-6 sm:p-8"><span className="grid size-12 place-items-center rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-950/40"><FiCheck /></span><h2 className="mt-5 text-3xl font-bold">Interview completed</h2><p className="mt-2 text-slate-500">Your answers have been saved for review and optional AI-generated feedback.</p><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Summary label="Type" value={session.interviewType} /><Summary label="Difficulty" value={session.difficulty} /><Summary label="Answered" value={`${session.answeredQuestions} / ${session.totalQuestions}`} /><Summary label="Unanswered" value={String(unanswered)} /></div><p className="mt-5 text-sm text-slate-500">Completed {formatDate(session.completedAt)}</p></Card>
